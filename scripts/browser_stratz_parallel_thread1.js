/*
 * STRATZ Parallel Fetcher - THREAD 1
 * Fetches first half of missing matches
 * API Key 1
 */
...
(async function() {
  ...
  console.log(`🚀 ${THREAD_NAME} - Fetches FIRST HALF of missing matches`);
  ...
  // THREAD 1: Takes first half
  const halfPoint = Math.floor(allMissing.length / 2);
  const myMatches = allMissing.slice(0, halfPoint);
  const toFetch = myMatches.slice(0, MAX_PER_THREAD);
  
  console.log(`🎯 ${THREAD_NAME} will fetch matches 0 to ${myMatches.length} (${toFetch.length} respecting daily limit)`);
  ...
  for (let i = 0; i < toFetch.length; i += BATCH_SIZE) {
    ...
    if (progress % 500 === 0) {
      ...
      console.log(`💾 [${THREAD_NAME}] Auto-saved to window.stratzCacheT1`);
    }
  ...
  console.log(`\n👉 To copy: copy(JSON.stringify(window.stratzCacheT1, null, 2))`);
